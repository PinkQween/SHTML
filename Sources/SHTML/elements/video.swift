/// Video type.
public struct Video: HTMLPrimitive, HTMLModifiable {
    /// Type alias.
    ...
    public func playsInline(_ value: Bool = true) -> Self {
        var copy = self
        if value { copy.attributes["playsinline"] = "" } else { copy.attributes.removeValue(forKey: "playsinline") }
        return copy
    }
    ...
    /// poster function.
    public func poster(_ path: String) -> Self {
        var copy = self
        copy.attributes["poster"] = path
        return copy
    }
}
